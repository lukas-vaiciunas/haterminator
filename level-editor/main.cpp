...
int main(void)
{
	...
	const ALLEGRO_COLOR clearColor = al_map_rgb(25, 25, 50);
	...
	while (!done)
	{
		...
		{
			driver.updateOnTick(keyboard);

			redraw = true;
		}
		else if (ev.type == ALLEGRO_EVENT_MOUSE_AXES)
		{
			mouse.updateOnAxes(ev);
		}
		else if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN)
		{
			mouse.updateOnPress(ev);

			driver.updateOnMousePress(mouse);
		}
		else if (ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP)
		{
			mouse.updateOnRelease(ev);
		}
		else if (ev.type == ALLEGRO_EVENT_KEY_DOWN)
		{
			keyboard.updateOnPress(ev);

			driver.updateOnKeyPress(keyboard);
		}
		else if (ev.type == ALLEGRO_EVENT_KEY_UP)
		{
			keyboard.updateOnRelease(ev);
		}
